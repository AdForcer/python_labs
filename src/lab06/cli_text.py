...
Пути относительно корня проекта python_labs/
    ...
    cat_parser.add_argument(
        "-n", "--number", action="store_true", help="нумеровать строки вывода"
    )

    # Подкоманда stats
    stats_parser = subparsers.add_parser(
        "stats", help="проанализировать частоты слов в тексте"
    )
    stats_parser.add_argument(
        "--input", dest="input_file", required=True, help="путь к текстовому файлу"
    )
    stats_parser.add_argument(
        "--top",
        type=int,
